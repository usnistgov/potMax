...
is justified. They are used for such purposes in many papers,
e.g., \cite{smith89, smith04, coles04, pintar_et_al15}. The original
theoretical justification is provided by
\cite{pickands71}.
...
which the $W$-statistics may again be leveraged. Let $\delta_i$ be the
...
interval for the threshold $b_i$. A natural transformation of the
$\delta_i$ is
\begin{align}
 w_i = \frac{\exp\{-\tau\delta_i\}}{\sum_i\exp\{-\tau\delta_i\}}
\end{align}
With $\tau = 0$, each threshold recieves equal weight, and
as $\tau$ approaches $\infty$, the weight corresponding to the
smallest value of $\delta$ approaches unity while the rest approach
...
\item Select the threshold or thresholds
  \begin{enumerate}
  \item Construct a set of potential thresholds. This may be done by by
  ...
  are $b_1 > b_2 > \cdots > b_{B - 1} > b_B$ such that if $b_i$
  corresponds to $n_i$ observations, $b_{i + 1}$ corresponds to $n_i +
...
name \texttt{jp1tap1715wind270}. The present sections describes
